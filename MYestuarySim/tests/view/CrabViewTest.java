...
public class CrabViewTest {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		try { 
			img = ImageIO.read(new File("./img/Crab Front.png"));
        } catch (IOException ex) {
    	   System.out.println("Image read error");
        }
		img = Helper.toBufferedImage(img.getScaledInstance((int)screenSize.getWidth()/(38+12), -1,1));
	}
	...
	public void imagePixelComparisonTest() {
		...
		for(int i=0;i<100;i++) {
			...
			assertEquals("The integer pixels should be equal",img.getRGB(rx, ry),cvi.getRGB(rx, ry));
		}
	}
}
